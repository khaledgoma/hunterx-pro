...
app.config["MAIL_USERNAME"] = os.environ.get(
    "HUNTERX_MAIL_USER", "khaledalqinas@gmail.com"
)
...
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///" + os.path.expanduser(
    "~/hunterx/web_dashboard/users.db"
)
...
class User(db.Model):
    ...
    scans = db.relationship(
        "ScanHistory", backref="user", cascade="all, delete-orphan", lazy=True
    )
    notifications = db.relationship(
        "Notification", backref="user", cascade="all, delete-orphan", lazy=True
    )
...
with app.app_context():
    ...
    if not User.query.filter_by(username="admin").first():
        admin = User(
            username="admin",
            email="admin@hunterx.com",
            password_hash=generate_password_hash(
                "HunterX2026!", method="pbkdf2:sha256:600000"
            ),
            plan="enterprise",
            assets_limit=999,
            is_active=True,
...
def login_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if user.plan == "trial":
            ...
            if days_passed >= 3:
                flash(
                    "Your 3-day free trial has expired. Upgrade to continue.", "warning"
                )
...
def register():
    if request.method == "POST":
        ...
        try:
            send_notification(
                "🎉 Welcome!",
                f"Thanks for joining, {username}! Your 3-day trial starts now.",
                "👋",
                user_id=user.id,
            )
            send_notification(
                "🆕 New User Registered",
                f"<b>{username}</b> ({email}) just joined.",
                "👤",
            )
        except Exception:
        ...
        flash(
            f"Welcome to HunterX, {username}! Your 3-day free trial is active.",
            "success",
        )
        return redirect(url_for("dashboard"))
...
def dashboard():
    user = get_current_user()
    scans = (
        ScanHistory.query.filter_by(user_id=user.id)
    ...
    stats = {
        ...
        "last_scan": (
            scans[0].scanned_at.strftime("%Y-%m-%d %H:%M") if scans else "No scans yet"
        ),
        "targets": (
            list(set(s.target for s in scans))[-5:] if scans else ["example.com"]
        ),
...
def start_scan():
    ...
    if user.plan == "trial":
        flash(
            "Manual scanning is only available for paid plans. Please upgrade!",
            "warning",
        )
        return redirect(url_for("pricing"))
    ...
    if current_assets >= user.assets_limit:
        flash(
            f"Asset limit reached ({user.assets_limit}). Upgrade your plan!", "warning"
        )
        return redirect(url_for("pricing"))
    demo_findings = [
        {
    ...
    send_notification(
        "🔍 Scan Completed",
        f"Scan of <b>{target}</b> completed with 3 findings.",
        "🎯",
        user_id=user.id,
    )
...
def generate_manual_report():
    ...
    if user.plan == "trial":
        flash(
            "Manual report generation is only available for paid plans. Please upgrade!",
            "warning",
        )
        return redirect(url_for("pricing"))
    ...
    send_notification(
        "📋 Report Generated",
        f"Manual report for <b>{program}</b> created.",
        "📄",
        user_id=user.id,
    )
...
def affiliate_dashboard():
    ...
    clicks_log = (
        AffiliateClickLog.query.order_by(AffiliateClickLog.clicked_at.desc())
        .limit(20)
        .all()
    )
    total_clicks = db.session.query(db.func.sum(AffiliateLink.clicks)).scalar() or 0
    total_conversions = (
        db.session.query(db.func.sum(AffiliateLink.conversions)).scalar() or 0
    )
    total_earnings = (
        db.session.query(db.func.sum(AffiliateLink.earnings)).scalar() or 0.0
    )
    affiliates_dict = {
        l.affiliate_id: {
            "name": l.name,
            ...
            "earnings": l.earnings,
            "link": f"{request.url_root}landing?ref={l.affiliate_id}",
        }
        for l in links
    }
    recent_clicks = [
        {
            ...
            "ip": c.ip_address,
            "timestamp": c.clicked_at.strftime("%Y-%m-%d %H:%M:%S"),
        }
        for c in clicks_log
    ]
    return render_template(
        "affiliates.html",
        affiliates=affiliates_dict,
...
def add_affiliate():
    ...
    affiliate_id = hashlib.md5(f"{name}_{secrets.token_hex(4)}".encode()).hexdigest()[
        :8
    ]
    new_link = AffiliateLink(
        affiliate_id=affiliate_id, name=name, commission_rate=commission
    )
    ...
    send_notification(
        "🤝 New Affiliate Added",
        f"<b>{name}</b> joined with <b>{commission}%</b> commission.",
        "💰",
    )
...
def upgrade_plan():
    flash(
        "Direct upgrades are currently disabled. Please complete your purchase via the Pricing page to unlock your plan.",
        "info",
    )
    return redirect(url_for("pricing"))
...
def admin_panel():
    return render_template(
        ...
        affiliates={
            ...
            for l in AffiliateLink.query.all()
        },
        total_clicks=db.session.query(db.func.sum(AffiliateLink.clicks)).scalar() or 0,
        total_conversions=db.session.query(
            db.func.sum(AffiliateLink.conversions)
        ).scalar()
        or 0,
        total_earnings=db.session.query(db.func.sum(AffiliateLink.earnings)).scalar()
        or 0.0,
        recent_users=User.query.order_by(User.created_at.desc()).limit(10).all(),
        recent_scans=ScanHistory.query.order_by(ScanHistory.scanned_at.desc())
        .limit(10)
        .all(),
        recent_notifs=Notification.query.order_by(Notification.created_at.desc())
        .limit(10)
        .all(),
        recent_clicks=[
            {
            ...
            for c in AffiliateClickLog.query.order_by(
                AffiliateClickLog.clicked_at.desc()
            )
            .limit(20)
            .all()
        ],
